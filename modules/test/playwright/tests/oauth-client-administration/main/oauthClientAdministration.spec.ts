...
test.afterEach(async ({AuthServerLocalMetadatasPage, OAuthClientClientsPage, viewAttributesPage}) => {	
	...
	test(
		...
		async ({addCustomFieldPage, OAuthClientClientsPage,}) => {
			...
			const oauthClientEntry: TOAuthClientEntry = {
				authServerWellKnownURI: 'https://accounts.google.com/.well-known/openid-configuration',
			...
			const newOAuthClientEntry = await OAuthClientClientsPage.addOAuthClientEntry(oauthClientEntry);

			oauthClientsCreated = true;

			await OAuthClientClientsPage.goToEntry(oauthClientEntry);

			await expect(OAuthClientClientsPage.matcherField).toHaveValue('screenName');

			if(newOAuthClientEntry?.customClaims) {
				 for (const customClaim of newOAuthClientEntry.customClaims) {
					await OAuthClientClientsPage.checkCustomClaimValues(customClaim);
				}
			}
		}
	);
