...
export class OAuthClientClientsPage {
    ...
    constructor(page: Page) {
        ...
        this.customClaimInput = page.getByRole('textbox', { name: 'Custom Claim' });
        this.customFieldInput = page.getByLabel('User Custom Fields');
        this.infoJSON = page.getByRole('textbox', { name: 'OAuth Client Information' });
        ...
        this.oidcUserInfoMapperJSON =  page.getByRole('textbox', { name: 'OpenId Connect User' });
        this.page = page;
        this.saveButton = page.getByRole('button', {name: 'Save'});
        this.wellKnownURI = page.getByRole('textbox', { name: 'Authorization Server Well-' });
    }

    async addOAuthClientEntry(oauthClientEntry: TOAuthClientEntry, dialogMessage?: string) {
        ...
        if (oauthClientEntry.authServerWellKnownURI) {
            await this.wellKnownURI.fill(oauthClientEntry.authServerWellKnownURI);
        }
        ...
        if (oauthClientEntry.customClaims) {
           oauthClientEntry.customClaims = await this.fillCustomClaims(oauthClientEntry.customClaims);
        }

        if (oauthClientEntry.oidcUserInfoMapperJSON) {
            await this.fillJSON(this.oidcUserInfoMapperJSON, oauthClientEntry.oidcUserInfoMapperJSON);
        } 
        ...
        await expect(this.page.getByText('Your request completed successfully')).toBeVisible();
    ...
    async fillJSON(jsonLocator: Locator, incomingData: any) {
        ...
        await jsonLocator.fill(
            JSON.stringify(
                await this.updateJSON(JSON.parse(await jsonLocator.inputValue()), incomingData), 
                null,
                2)
        );
    }
    ...
    async fillCustomClaims(customClaims: TCustomClaim[]) {
        ...
        for (const customClaim of customClaims) {
            ...
            await expect(
                this.page.locator(`[id="_com_liferay_oauth_client_admin_web_internal_portlet_OAuthClientAdminPortlet_customClaimsKey-${index + 1}"]`))
                .toBeVisible();

            await this.page.locator(`[id="_com_liferay_oauth_client_admin_web_internal_portlet_OAuthClientAdminPortlet_customClaimsKey-${index}"]`).selectOption(customClaim.expandoColumnName);

            await this.page.locator(`[id="_com_liferay_oauth_client_admin_web_internal_portlet_OAuthClientAdminPortlet_customClaimsValue-${index}"]`).fill(customClaim.oidcProviderCustomClaim);
    ...
    async updateJSON(jsonObject: any, incomingData: any)  {
        Object.entries(incomingData).forEach(async ([key, value]) => {
            if (value !== undefined && value !== null) {
                if (typeof jsonObject[key] === 'object' && !Array.isArray(value)) {
    ...
    async deleteOAuthClientEntries() {
        ...
        const row = this.oAuthClientsTable
            .getByRole('row')
            .last();
    ...
    async goToEntry(oauthClientEntry: TOAuthClientEntry) {
        ...
        await this.page.getByRole('link', { name: oauthClientEntry.infoJSON?.client_id }).click();
    ...
    async checkCustomClaimValues(customClaim: TCustomClaim) {
        await expect(
            this.page.locator(`select[id*="customClaimsKey"]:has(option:checked:text("${customClaim.expandoColumnName}"))`))
            .toBeVisible();
          
        await expect(
            this.page.locator(`input[id*="customClaimsValue"][value="${customClaim.oidcProviderCustomClaim}"]`))
            .toBeVisible();
    }
}
