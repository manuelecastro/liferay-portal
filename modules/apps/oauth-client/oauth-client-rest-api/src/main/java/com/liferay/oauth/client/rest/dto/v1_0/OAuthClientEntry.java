...
public class OAuthClientEntry implements Serializable {
	...
	@io.swagger.v3.oas.annotations.media.Schema
	@Valid
	public CustomClaim[] getCustomClaims() {
	...
	public void setCustomClaims(CustomClaim[] customClaims) {
	...
	public void setCustomClaims(
		UnsafeSupplier<CustomClaim[], Exception> customClaimsUnsafeSupplier) {
	...
	@GraphQLField
	@JsonProperty(access = JsonProperty.Access.READ_WRITE)
	protected CustomClaim[] customClaims;

	@JsonIgnore
	private Supplier<CustomClaim[]> _customClaimsSupplier;
	...
	public String toString() {
		...
		CustomClaim[] customClaims = getCustomClaims();
		...
		if (customClaims != null) {
			...
			for (int i = 0; i < customClaims.length; i++) {
				sb.append(String.valueOf(customClaims[i]));

				if ((i + 1) < customClaims.length) {
					sb.append(", ");
				}
			}
